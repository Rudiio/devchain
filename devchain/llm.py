...
def load_Azure_OpenAI(model:str='gpt-35-turbo',temperature:float=0.0)->AzureChatOpenAI:
    ...
    if model == 'gpt-4':
        ...
        return AzureChatOpenAI(azure_endpoint=endpoint,
                              azure_deployment='gpt-4',
                              model='gpt-4',
                              temperature=temperature,)

    endpoint = os.environ['AZURE_OPENAI_ENDPOINT_35']
    llm = AzureChatOpenAI(azure_endpoint=endpoint,
                          azure_deployment='gpt-35-turbo',
                          model='gpt-35-turbo',
                          temperature=temperature,)
    return llm
        
def load_Hugging_face(repo_id="mistralai/Mistral-7B-Instruct-v0.2",temperature:float=0.0)->HuggingFaceHub:
