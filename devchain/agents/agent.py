...
from devchain.llm import load_OpenAI
from devchain.prompts.core import create_agent_prompt
from devchain.rag.rag import Rag
from devchain.utils.tracer import Tracer, load_callback
...
class Agent(BaseModel):
    ...
    target_document: str = Field(
        description="Document that will nbe produced by the agent"
    )
    role: str = Field(description="Role of the agent. ex: Product Owner")
    goals: str = Field(
        description="Description of the main goals that the agent need to do. ex : Write code, ... "
    )
    team_description: Optional[str] = Field(
        default="", description="Description of the team members."
    )
    team_goal: Optional[str] = Field(
        default="",
        description="Description of the goal of the team on a particular project. ex: Build a timesheet application.",
    )
    agent: InstanceOf[Runnable] = Field(
        default=None, description="Langchain runnable agent that need to be executed"
    )
    llm: Union[InstanceOf[BaseChatModel], InstanceOf[BaseLLM]] = Field(
        default_factory=load_OpenAI, description="LLM model that powers the agent"
    )
    memory: list = Field(default=[], description="Buffer that stores the agent memory")
    memory_bool: bool = Field(
        default=False,
        description="True if the agents need to have memories of their actions",
    )
    prompt: InstanceOf[BasePromptTemplate] = Field(
        default=None, description="Base agent prompts that needs to be completed"
    )
    iterations: int = Field(
        default=0, description="Number of iterations in the agent(number of invocation)"
    )
    working_message: Message = Field(default=None, description="Message to work on")
    max_memory: int = Field(
        default=2, description="Maximum exchanges storable in the memory"
    )
    streaming: bool = True
    working_dir: str = Field(
        default="", description="Folder in which the agents need to save their work"
    )
    rag: InstanceOf[Rag] = Field(
        default=None, description="Rag instance to perform rag"
    )
    callback: InstanceOf[CallbackHandler] = Field(
        default_factory=load_callback,
        description="Langfuse callback for open source tracing",
    )
    tracer: Tracer = Field(
        default=None, description="Traces the statistics of the application"
    )
    test_logic: bool = False
    websocket: Union[InstanceOf[WebSocket], None] = Field(
        default=None, description="Websocket that connects the backend to the front-end"
    )
    ...
    def initialize_agent(
        ...
        self.prompt = create_agent_prompt(
            self.role, self.goals, self.team_description, self.team_goal
        )

        # Chain entry arguments
        chain_args = {
            "mission": lambda x: x["mission"],
    ...
    def invoke(self, mission: str, context: str = "") -> str:
        ...
        args = {"mission": mission, "chat_history": self.memory, "context": context}
        ...
        if self.memory_bool:
            ...
            self.memory.extend(
                [HumanMessage(content=mission), AIMessage(content=output)]
            )
    ...
    async def stream(
        self, mission: str, verbose=True, context: str = "", prefix: str = ""
    ):
        ...
        if self.is_connected():
            streaming_generator = self.agent.astream(
                args, config={"callbacks": [self.callback]}
            )
            msg = WsMessage.create_msg(
                object=" start streaming",
                action="processing",
            ...
            async for chunk in streaming_generator:
                msg = await WsMessage.acreate_msg(
                    object="Streaming tokens",
                    action="processing",
                    document="TODO",
                    model=self.llm.model_name,
                    agent=self.job,
        ...
        # Printing on the terminal
            ...
            streaming_generator = self.agent.stream(
                args, config={"callbacks": [self.callback]}
            )
            print("\n")
        ...
        rich.print(
            f":white_check_mark: Finished [bold green]\[{time.time() - start : .3f} s][/bold green]"
        )
    ...
    async def execute(
        ...
        if self.streaming:
            result = await self.stream(
                mission=mission, verbose=verbose, context=context, prefix=prefix
            )
        else:
            result = self.invoke(
                mission=mission,
